...
Pipeable APIs provide flexibility for adding new behavior and creating new functions for working with data types. With the `pipe` function from `fp-ts`, you can have usages with good inference.
...
Since the last parameter is an array, we can inject this function anywhere inside of a `pipe` chain with an an Array:
